package br.com.poison.arcade.duels.game.list.punctuation.sumo;
...
public class Sumo extends Game {
    ...
    public Sumo(Duels instance, Integer minRooms, Integer maxRooms, String mapsDirectory) {
        super(instance, minRooms, maxRooms, mapsDirectory, ArcadeCategory.SUMO);

        setInfo(Arena.TeamInfoSidebar.POINTS);

        setAllowedTakeLife(false);
        setAllowedMoveOnStarting(false);
    ...
    public void loadSidebar(Client client) {
        ...
        if (!arena.hasCondition(RoomCondition.PLAYING)) {
            ...
            arena.sendTeamInfoInSidebar(sidebar, Arena.TeamInfoSidebar.POINTS);
        }
        ...
        if (!client.isPlayer()) {
            ...
            sidebar.addRow("streak", "Winstreak: ", "§7" + Util.formatNumber(client.getMember().getStats().getSumo().getStreak()));
        }
    ...
    public void updateTeamStatsAfterEnd(Team winner, List<Team> losers) {
        for (Player player : winner.getPlayers()) {
            ...
            DefaultStats sumo = member.getStats().getSumo();

            sumo.setWins(sumo.getWins() + 1);
            sumo.setStreak(sumo.getStreak() + 1);

            if (sumo.getStreak() > sumo.getBestStreak())
                sumo.setBestStreak(sumo.getStreak());
        ...
        for (Team loser : losers) {
            ...
            for (Player player : loser.getPlayers()) {
                ...
                sumo.setDefeats(sumo.getDefeats() + 1);
                sumo.setStreak(0);
    ...
    public void onWater(PlayerMoveEvent event) {
        Client client = getClient().read(event.getPlayer().getUniqueId());

        Material self = event.getTo().getBlock().getRelative(BlockFace.SELF).getType();

        if (self.name().contains("WATER") && inGameAndCondition(client, RoomCondition.PLAYING))
            handleDeath(client, client.getArena(), DeathCause.VOID);
    }
}
