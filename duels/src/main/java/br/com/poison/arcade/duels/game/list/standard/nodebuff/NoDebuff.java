package br.com.poison.arcade.duels.game.list.standard.nodebuff;
...
public class NoDebuff extends Game {
    ...
    public NoDebuff(Duels instance, Integer minRooms, Integer maxRooms, String mapsDirectory) {
        super(instance, minRooms, maxRooms, mapsDirectory, ArcadeCategory.NODEBUFF);
    ...
    public void updateTeamStatsAfterEnd(Team winner, List<Team> losers) {
        for (Player player : winner.getPlayers()) {
            ...
            DefaultStats nodebuff = member.getStats().getNodebuff();

            nodebuff.setWins(nodebuff.getWins() + 1);
            nodebuff.setStreak(nodebuff.getStreak() + 1);

            if (nodebuff.getStreak() > nodebuff.getBestStreak())
                nodebuff.setBestStreak(nodebuff.getStreak());
        ...
        for (Team loser : losers) {
            ...
            for (Player player : loser.getPlayers()) {
                ...
                nodebuff.setDefeats(nodebuff.getDefeats() + 1);
                nodebuff.setStreak(0);
