package br.com.poison.arcade.duels.game.list.standard.uhc;
...
public class UHC extends Game {
    ...
    public UHC(Duels instance, Integer minRooms, Integer maxRooms, String mapsDirectory) {
        super(instance, minRooms, maxRooms, mapsDirectory, ArcadeCategory.UHC);
    ...
    public void updateTeamStatsAfterEnd(Team winner, List<Team> losers) {
        for (Player player : winner.getPlayers()) {
            ...
            DefaultStats uhc = member.getStats().getUhc();

            uhc.setWins(uhc.getWins() + 1);
            uhc.setStreak(uhc.getStreak() + 1);

            if (uhc.getStreak() > uhc.getBestStreak())
                uhc.setBestStreak(uhc.getStreak());
        ...
        for (Team loser : losers) {
            ...
            for (Player player : loser.getPlayers()) {
                ...
                uhc.setDefeats(uhc.getDefeats() + 1);
                uhc.setStreak(0);
